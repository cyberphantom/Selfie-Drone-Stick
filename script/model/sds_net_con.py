...
class DDDQNNet:
    def __init__(self, action_size, learning_rate, name='DQNetwork'):
        ...
        with tf.variable_scope(name):
            ...
            # [None, 84, 84, 4]
            self.inputs_ = tf.placeholder(tf.float32, [None, 1, 8, 4], name="inputs")
            ...
            self.value = tf.layers.dense(inputs=self.value_fc2,
                                         units=1,
                                         activation=None,
                                         kernel_initializer=tf.contrib.layers.xavier_initializer(),
                                         name="value")
            ...
            self.advantage_fc = tf.layers.dense(inputs = self.value_fc2,
                                  units = 256,
                                  activation = tf.nn.elu,
                                       kernel_initializer=tf.contrib.layers.xavier_initializer(),
                                name="advantage_fc1")
